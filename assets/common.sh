...
get_current_version() {
  ...
  echo "$folder_contents" | jq --arg v "$regex" '[.[].path | capture($v)]' | jq 'sort_by(.version | split("."))' | jq '[.[length-1] | {version: .version}]'
}
...
get_all_versions() {
  ...
  echo "$folder_contents" | jq --arg v "$regex" '[.[].path | capture($v)]' | jq 'sort_by(.version | split("."))' | jq '[.[] | {version: .version}]'
}
...
get_files() {
  ...
  echo "$folder_contents" | jq --arg v "$regex" '[.[].path | capture($v)]' | jq 'sort_by(.version  | split("."))' | jq '[.[] | {path: .path, version: .version}]'
}
